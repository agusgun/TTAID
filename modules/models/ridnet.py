...
class RIDNet(BaseModel):
    def __init__(self, args, device):
        super(RIDNet, self).__init__(args, device)

        self.restoration_net, self.rec_criterion = build_components(
            self.args
        )
        ...
        self.logger = logging.getLogger("RIDNet Model")
    ...
    def train_one_epoch(self, train_loader, epoch):
        ...
        tqdm_batch = tqdm(
            train_loader,
            desc="epoch-{} training".format(self.current_epoch),
        )
        ...
        for curr_it, data in enumerate(tqdm_batch):
            ...
            tqdm_batch.set_description(
                "Training: ({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | LossC: {lossC:.4f}".format(
                    batch=curr_it + 1,
        ...
        self.logger.info(
            "Training at epoch-{} | LR: {} LossC: {}".format(
    ...
    def init_training_logger(self):
        ...
        self.summary_writer = SummaryWriter(
            log_dir=self.args.summary_dir, comment="RIDNet"
        )
